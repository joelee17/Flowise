...
class WorkdayAgentGateway_Agentflow implements INode {
    label: string
    name: string
    version: number
    description: string
    ...
    constructor() {
        this.label = 'Workday Agent Gateway'
        this.name = 'workdayAgentGateway'
        this.version = 1.0
        this.type = 'WorkdayAgentGateway'
        this.category = 'Agent Flows'
        this.description = 'Gateway for Workday API requests with specialized handling'
        this.baseClasses = [this.type]
        this.color = '#4A90E2'
        this.icon = 'workdayMCP_circular.png'
        this.credential = {
    ...
    async run(nodeData: INodeData, _: string, options: ICommonObject): Promise<any> {
        ...
        try {
            ...
            console.error('Workday Agent Gateway Request Error:', error)

            const errorMessage =
                error.response?.data?.message || error.response?.data?.error || error.message || 'An error occurred during the Workday request'
...
module.exports = { nodeClass: WorkdayAgentGateway_Agentflow }
